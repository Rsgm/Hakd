...
public final class Internet {
    ...
    private void generateBackbones(int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            @SuppressWarnings("deprecation") BackboneProviderNetwork b = new BackboneProviderNetwork(this);
    ...
    private void generateIsps(int amount) {
        for (int i = 0; i < amount; i++) {
            @SuppressWarnings("deprecation") InternetProviderNetwork s = new InternetProviderNetwork(this);
        ...
        for (InternetProviderNetwork i : internetProviderNetworks) {
            if (i.getMasterDns().getIp()[1] == ip[1]) {
                for (Connection c : i.getMasterDns().getConnections()) {
                    if (c.getClient().getIp()[2] == ip[2] && c.getClient().getType() == DeviceType.ROUTER) {
