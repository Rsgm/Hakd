...
public class DefaultNetwork implements Network {
	...
	case BUSINESS: // company // random company
	    ...
	    break;
	case ISP:
	    ipRegion = IpRegion.values()[internet.getInternetProviderNetworks()
		    .size() % IpRegion.values().length];
	    serverLimit = (int) ((level + 1) * (Math.random() * 3 + 1));
	    level = (int) (Math.random() * 8);
	    dnsLimit = 1;
	    routerLimit = 1;
	    sphere = modelBuilder.createSphere(15f, 15f, 15f, 20, 10,
		    new Material(ColorAttribute.createDiffuse(Color.CYAN)),
		    Usage.Position | Usage.Normal);
	    break;
	case BACKBONE:
	    ipRegion = IpRegion.values()[internet.getBackboneProviderNetworks()
		    .size() % IpRegion.values().length];
	    serverLimit = (int) ((level + 1) * (Math.random() * 3 + 1));
	    level = (8);
	    dnsLimit = 1;
	    routerLimit = 1;
	    sphere = modelBuilder.createSphere(25f, 25f, 25f, 20, 10,
		    new Material(ColorAttribute.createDiffuse(Color.RED)),
		    Usage.Position | Usage.Normal);
	    break;
	...
	case BACKBONE:
	    ...
	    router.setIp(new short[] { Dns.generateIpByte(ipRegion),
		    Dns.generateIpByte(IpRegion.none),
		    Dns.generateIpByte(IpRegion.none), 1 });
	    ...
	    for (BackboneProviderNetwork b : internet
		    .getBackboneProviderNetworks()) {
		b.getRouters()
			.get(0)
			.Connect(
				router,
				new Port(Protocol.DNS.name(),
					Protocol.DNS.portNumber, Protocol.DNS));
	    }
	    break;
	case ISP:
	    router.setParentNetwork(internet.getBackboneProviderNetworks().get(
		    internet.getInternetProviderNetworks().size()
			    % internet.getBackboneProviderNetworks().size()));
	    router.setParent(router.getParentNetwork().getMasterDns());

	    position = new Vector3(
		    router.getParentNetwork().getPosition().x
			    + (float) ((Math.random() * BackboneRegionSize) - BackboneRegionSize / 2),
		    router.getParentNetwork().getPosition().y
			    + (float) ((Math.random() * BackboneRegionSize) - BackboneRegionSize / 2),
		    (router.getParentNetwork()).getPosition().z
			    + (float) ((Math.random() * BackboneRegionSize) - BackboneRegionSize / 2));

	    router.setIp(router.getParentNetwork().register(router, 1));
	    break;
	default:
	    router.setParentNetwork(internet.getInternetProviderNetworks().get(
		    (int) (Math.random() * internet
			    .getInternetProviderNetworks().size())));
	    router.setParent(router.getParentNetwork().getMasterDns());

	    position = new Vector3(
		    router.getParentNetwork().getPosition().x
			    + (float) ((Math.random() * ispRegionSize) - ispRegionSize / 2),
		    router.getParentNetwork().getPosition().y
			    + (float) ((Math.random() * ispRegionSize) - ispRegionSize / 2),
		    router.getParentNetwork().getPosition().z
			    + (float) ((Math.random() * ispRegionSize) - ispRegionSize / 2));
