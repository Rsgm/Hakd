...
public class TerminalInput implements InputProcessor {
	...
	void ClearInput() {
		text = "Boot>";/*terminal.getDevice().getNetwork().getIp() + ">";*/
		cursor = text.length();
	}
	...
	public boolean scrolled(int amount) { // TODO change these values
		...
		if ((terminal.getScroll() >= Gdx.graphics.getHeight() / 2 + 10 || amount == -1)
				&& (terminal.getScroll() <= -Gdx.graphics.getHeight() / 2 + 18 + 16 * terminal.getText().size() || amount == 1)) {
			terminal.setScroll(terminal.getScroll() + amount * 10);
			System.out.println(amount); // maybe move the text originY up/down based on the scroll
		}
		return true;
	}
